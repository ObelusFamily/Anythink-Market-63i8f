...
const Comment = props => {
	...
	const show = props.currentUser && props.currentUser.username === comment.seller.username;
	return (
			...
			<div className='card m-2 shadow-sm' style={{ minHeight: '200px' }}>
				<div className='card-body d-flex flex-column'>
					...
					<div className='d-flex flex-row align-items-center pt-2'>
						...
						&nbsp;
						<Link to={`/@${comment.seller.username}`}>{comment.seller.username}</Link>
						<span className='text-light mx-2'>|</span>
						<span className='flex-grow-1'>{new Date(comment.createdAt).toDateString()}</span>
